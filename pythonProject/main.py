...
class Sentiment(object):
    ...
    def get_last_rec_comment(self):
        ...
        try:
            if conn.open:
                with conn.cursor() as cursor:
                    # Read data from database
                    sql = "SELECT * FROM `drl_comment` WHERE id=(SELECT max(id) FROM `drl_comment`)"
                    cursor.execute(sql)
    ...
    def get_data_from_database(self, r):
        ...
        for dg in danh_gias:
            ...
            print(x)
            x = get_clean(x)
            self.phan_loai_sentiment(x, dg['hoat_dong_id'], dg['user_id'], r)

    def phan_loai_sentiment(self, noiDung, idThucAn, idNguoiDung, idComment):
        x = noiDung
        print(x)
        ...
        # New data to append
        new_data = {'Review': [x], 'Sentiment': [tmp], 'idThucAn': idThucAn, 'idNguoiDung': idNguoiDung, 'idComment': idComment}
...
# rec.similarity()
# sentiment_object.get_data_from_database(rec)
...
def get_sentiment():
    ...
    if request.method == "POST":
        ...
        sentiment_object.phan_loai_sentiment(str(s), idThucAn, idNguoiDung, idComment)
    return str(s)
...
async def refresh_data(sen):
    sen.df = pd.read_excel(sen.existing_file)
    sen.X = sen.tfidf.fit_transform(sen.df['Review'].values.astype('U'))
    sen.y = sen.df['Sentiment']
    sen.X_train, sen.X_test, sen.y_train, sen.y_test = train_test_split(sen.X, sen.y, test_size=0.2,
                                                                            random_state=0)
    sen.clf = LinearSVC().fit(sen.X_train, sen.y_train)
    sen.y_pred = sen.clf.predict(sen.X_test)


async def rd(sen):
    input_task = asyncio.create_task(refresh_data(sen))
    await input_task


@app.route("/refreshdata/", methods=['GET'])
def get_refresh_data():
    asyncio.run(rd(sentiment_object))
    return "refresh_data"
